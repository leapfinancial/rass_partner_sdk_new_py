# coding: utf-8

"""
    

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from raassdkpy.models.validate_otp403_response import ValidateOTP403Response  # noqa: E501

class TestValidateOTP403Response(unittest.TestCase):
    """ValidateOTP403Response unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ValidateOTP403Response:
        """Test ValidateOTP403Response
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ValidateOTP403Response`
        """
        model = ValidateOTP403Response()  # noqa: E501
        if include_optional:
            return ValidateOTP403Response(
                code = '',
                reason = ''
            )
        else:
            return ValidateOTP403Response(
                code = '',
                reason = '',
        )
        """

    def testValidateOTP403Response(self):
        """Test ValidateOTP403Response"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
