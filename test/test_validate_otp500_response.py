# coding: utf-8

"""
    

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from raassdkpy.models.validate_otp500_response import ValidateOTP500Response  # noqa: E501

class TestValidateOTP500Response(unittest.TestCase):
    """ValidateOTP500Response unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ValidateOTP500Response:
        """Test ValidateOTP500Response
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ValidateOTP500Response`
        """
        model = ValidateOTP500Response()  # noqa: E501
        if include_optional:
            return ValidateOTP500Response(
                remaining_attempts = 1.337,
                code = '',
                reason = ''
            )
        else:
            return ValidateOTP500Response(
                remaining_attempts = 1.337,
                code = '',
                reason = '',
        )
        """

    def testValidateOTP500Response(self):
        """Test ValidateOTP500Response"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
